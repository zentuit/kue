...
function Worker( queue, type ) {
  ...
  this.client.joba_name = 'Shared Worker client';
  this.running = true;
  this.job     = null;
}
...
Worker.prototype.getJob = function( fn ) {
  ...
  client.joba_name = `getJob.${self.type}`;
  ...
  console.log(`-- Worker.blpop: ${self.type} :: err: ${err} :: client ${self.client.joba_name} (${self.client.connection_id}) is connected? ${c}`);
