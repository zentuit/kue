...
Worker.prototype.start = function( fn ) {
  ...
  console.log(`-- Worker.start: ${self.type}`);

  self.getJob(function( err, job ) {

  console.log(`-- Worker.getJob callback: ${self.type} :: err: ${err} :: ${job}`);
  ...
  console.log(`-- Worker.getJob callback - calling process: ${self.type}`);
...
Worker.prototype.getJob = function( fn ) {
  ...
  client.joba_name = self.type;
  console.log(`-- Worker.getJob: ${self.type} :: client ${client.joba_name} (${client.connection_id}) connected?: ${client.connected}`);

  // BLPOP indicates we have a new inactive job to process
  client.blpop(client.getKey(self.type + ':jobs'), 0, function( err ) {

  const c = self.client && self.client.connected;
  console.log(`-- Worker.blpop: ${self.type} :: err: ${err} :: client ${c.connection_id} is connected? ${c}`);
  ...
  console.log(`-- Worker.blpop (before zpop): ${self.type} :: is connected? ${c}`);

    self.zpop(self.client.getKey('jobs:' + self.type + ':inactive'), function( err, id ) {

  console.log(`-- Worker.zpop: ${self.type} :: err: ${err} :: id ${id}`);
